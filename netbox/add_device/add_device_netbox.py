...
def get_current_inventory(device_id):
    response = requests.get(f'{netbox_url}dcim/inventory-items/', headers=headers, params={'device_id': device_id})
...
def add_inventory_item_to_device(device_id, part_id, serial, name):
    ...
    response = requests.get(f'{netbox_url}dcim/inventory-items/', headers=headers, params={'serial': serial})
    if response.status_code == 200 and len(response.json()['results']) > 0:
    ...
    response = requests.post(f'{netbox_url}dcim/inventory-items/', headers=headers, json=inventory_data)
    if response.status_code == 201:
...
def remove_inventory_item(item_id):
    response = requests.delete(f'{netbox_url}dcim/inventory-items/{item_id}/', headers=headers)
    ...
    for network_range in network_ranges:
        ...
        for ip in network:
            ...
            try:
                ...
                # Regular expression to extract SFP, GLC, QSFP, SFBR, CVR, Unspecified information
