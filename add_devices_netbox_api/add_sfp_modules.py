    ...
    for network_range in network_ranges:
        ...
        for ip in network:
            ...
            try:
                ...
                output2 = net_connect.send_command('sh interface transceiver | egrep ''serial|type')
                ...
                regex2 = r"type is (\S+)\s+serial number is (\S+)"
                matches = re.findall(regex, output)
                matches2 = re.findall(regex2, output2)
                ...
                if matches2 and device_id:
                    for sfp_info in matches2:
                        sfp_type = sfp_info[0]
                        sfp_sn = sfp_info[1].strip()
                        new_inventory_serials.add(sfp_sn)

                        if sfp_sn not in current_inventory_serials:
                            add_inventory_item_to_device(device_id, sfp_type, sfp_sn, sfp_type)
