...
def add_user_to_device(ip, device_params, user):
    device_params['ip'] = ip
    try:
        # Connect to the device
        net_connect = ConnectHandler(**device_params)

        # Send command to add new user
        config_commands = [
            f"username {user['username']} privilege {user['privilege']} password {user['password']}"
        ]
        ...
        print(f"User {user['username']} added to {hostname} ({ip})")
...
with ThreadPoolExecutor(max_workers=5) as executor:
    for network_range in network_ranges:
        ...
        for ip in network:
            executor.submit(add_user_to_device, str(ip), device_params, new_user)
